...
Skipped entirely when ``dataspace`` is not installed: it is an optional
...
dataspace = pytest.importorskip("dataspace", reason="optional [dataspace] extra")

from dataspace import DataSpace, Health, Source, SourceInfo  # noqa: E402
...
class TestReadinessGate:
    ...
    def test_gate_fails_before_a_workflow_writes(self, tmp_path):
        ...
        with pytest.raises(dataspace.SourceNotReadyError) as exc:
            space.require_ready()
...
class TestStandaloneIndependence:
    def test_the_package_does_not_import_the_adapter(self):
        """Importing market_data_hub must not require dataspace: the repo has
        ...
        for module in package_dir.rglob("*.py"):
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module:
                    if node.module.split(".")[0] == "dataspace":
                        importers.append(module.name)
                    elif node.module.endswith("dataspace_source"):
                        importers.append(module.name)
                elif isinstance(node, ast.Import):
                    if any(a.name.split(".")[0] == "dataspace" for a in node.names):
                        importers.append(module.name)
        assert not importers, f"these modules would make dataspace mandatory: {importers}"
