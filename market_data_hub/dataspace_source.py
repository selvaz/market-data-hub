...
Makes market-data-hub registrable in a :class:`dataspace.DataSpace` so a
...
``dataspace`` is an optional dependency (``pip install
market-data-hub[dataspace]``). Nothing else in this package imports this
module, so the repo installs and runs without it. Note that ``dataspace``
...
from dataspace import Health, SourceInfo
...
class MarketDataSource:
    ...
    Satisfies the ``dataspace.Source`` protocol structurally — there is no
    ...
    def health(self) -> Health:
        ...
        """
        try:
            path = _resolve_db_path(self._db_path)
